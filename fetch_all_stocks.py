...
def get_all_stock_codes(date: str = None) -> list:
    """
    获取全市场股票代码列表
    自动回溯到最近交易日查询（周末/节假日交易日无数据）
    返回: ['sh.600000', 'sz.000001', ...]
    """
    # 从最近交易日向前回溯，最多回溯30天
    check_date = datetime.now() if date is None else datetime.strptime(date, "%Y-%m-%d")

    stock_list = []
    for days_back in range(30):
        d = (check_date - timedelta(days=days_back)).strftime("%Y-%m-%d")
        ...
        while rs.next():
            row = rs.get_row_data()
            raw_codes.append(row[0])

        # 交易日通常有5000+只，如果拿到超过500只说明是有效交易日
        if len(raw_codes) > 500:
            logger.info(f"使用 {d} 的交易日数据（回溯{days_back}天），获取到 {len(raw_codes)} 个代码")
            stock_list = raw_codes
            break

    if not stock_list:
        logger.error("回溯30天仍未找到有效交易日")
        return []

    # 过滤：只保留A股个股（排除指数、基金、债券）
    all_stocks = []
    ...
    logger.info(f"★ 全市场A股数: {len(all_stocks)} (含{len(indices)}个指数)")
    return all_stocks
...
def run_incremental_update(push: bool = False):
    ...
    for i, code in enumerate(stock_codes):
        ...
        if (i + 1) % 200 == 0 or (i + 1) == len(stock_codes):
            ...
            remaining = (len(stock_codes) - i - 1) / speed if speed > 0 else 0
            logger.info(
                f"  进度: {i + 1}/{len(stock_codes)} | 成功:{success_count} 失败:{fail_count} | 剩余:{remaining / 60:.1f}分钟")
